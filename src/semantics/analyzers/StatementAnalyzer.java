...
import execution.commands.controlled.*;
import execution.commands.simple.Print;
import execution.commands.simple.Return;
import execution.commands.simple.Scan;
...
public class StatementAnalyzer {
    ...
    public void analyze(StatementContext ctx) {
        ...
        //an IF statement
            ...
            //Console.log(LogType.DEBUG, "WhileCommand par expression: " +ctx.parExpression().getText());
    ...
    private void handlePrintStatement(StatementContext ctx) {
        ...
        Print printCommand = new Print(ctx.expression(0));
...
//        Scan scanCommand = new Scan(ctx.StringLiteral().getText(), ctx.Identifier().getText());
...
//        }
    ...
    private void handleReturnStatement(ExpressionContext exprCtx) {
        Return returnCommand = new Return(exprCtx, ExecutionManager.getInstance().getCurrentFunction());
		/*
    ...
    public static boolean isIFStatement(StatementContext ctx) {
        List<TerminalNode> tokenList = ctx.getTokens(FRIENDLYLexer.IF);

        return (tokenList.size() != 0);
    }

    public static boolean isELSEStatement(StatementContext ctx) {
        List<TerminalNode> tokenList = ctx.getTokens(FRIENDLYLexer.ELSE);

        return (tokenList.size() != 0);
    }

    public static boolean isWHILEStatement(StatementContext ctx) {
        List<TerminalNode> whileTokenList = ctx.getTokens(FRIENDLYLexer.WHILE);
        List<TerminalNode> doTokenList = ctx.getTokens(FRIENDLYLexer.DO);
    ...
    public static boolean isFORStatement(StatementContext ctx) {
        List<TerminalNode> forTokenList = ctx.getTokens(FRIENDLYLexer.FOR);

        return (forTokenList.size() != 0);
    }

    public static boolean isRETURNStatement(StatementContext ctx) {
        List<TerminalNode> returnTokenList = ctx.getTokens(FRIENDLYLexer.RETURN);
